...
echo strftime($formatoData2, $agora->getTimestamp()) . '<br>'; //é valido setlocale => format é ingles

$dataFixa = new DateTime('1975-01-25 15:30:50'); //construindo direto no construtor
echo strftime($formatoDataHora, $dataFixa->getTimestamp()) . '<br>';

$amanha = new DateTime('+2 week'); //passa string e já é convertido
echo strftime($formatoData2, $amanha->getTimestamp()) . '<br>';

$amanha->modify('+1 day');
echo strftime($formatoDataHora, $amanha->getTimestamp()) . '<br>'; //modificou => acrescentou mais um dia da variável $amanha

$amanha->setDate(2000, 5, 20);
echo strftime($formatoDataHora, $amanha->getTimestamp()) . '<br>'; //setando data específica
